<template>
  <div id="Technologies" class="px-10 sm:px-10 md:px-20 lg:px-30 xl:px-40">
    <h1 class="mx-auto dark:text-white text-lightgrey text-5xl sm:text-6xl md:text-7xl lg:text-8xl text-center font-extrabold border-b-8 border-indigo-600">Technologies</h1>
    <div class="grid grid-cols-4 sm:grid-cols-4 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-6 gap-12 place-items-center pt-8">
      <div><img src="~assets/img/svg/python.svg" alt="Python" /></div>
      <div><img src="~assets/img/svg/flask.svg" alt="Flask" /></div>
      <div><img src="~assets/img/svg/javascript.svg" alt="Javascript" /></div>
      <div><img src="~assets/img/svg/html-5.svg" alt="HTML" /></div>
      <div><img src="~assets/img/svg/css-3.svg" alt="CSS" /></div>
      <div><img src="~assets/img/svg/vue.svg" alt="Vue" /></div>
      <div><img src="~assets/img/svg/nuxt.svg" alt="Nuxt" /></div>
      <div><img src="~assets/img/svg/tailwindcss.svg" alt="Tailwind" /></div>
      <div><img src="~assets/img/svg/bulma.svg" alt="Bulma" /></div>
      <div><img src="~assets/img/svg/gitlab.svg" alt="Gitlab" /></div>
      <div><img src="~assets/img/svg/github.svg" alt="Github" /></div>
      <div><img src="~assets/img/svg/jira.svg" alt="Jira" /></div>
      <div><img src="~assets/img/svg/netlify.svg" alt="Netlify" /></div>
      <div><img src="~assets/img/svg/heroku.svg" alt="Heroku" /></div>
      <div><img src="~assets/img/svg/postman.svg" alt="Postman" /></div>
      <div><img src="~assets/img/svg/vs-code.svg" alt="VS Code" /></div>
    </div>
  </div>
</template>